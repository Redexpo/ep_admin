...
export default function BlogForm({ mode, initial }: BlogFormProps) {
    ...
    return (
        <div className="space-y-6 pb-16">
            ...
            {/* Metadata row: publish · category+tags · cover */}
            <div className="grid gap-6 lg:grid-cols-3">
                {/* Publish card */}
                <div className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
                    <h3 className="text-[13px] font-bold uppercase tracking-widest text-slate-400">Publish</h3>
                    <div className="mt-3 grid grid-cols-2 gap-1.5">
                        {STATUS_OPTIONS.map((opt) => (
                            <button
                                ...
                                className={`flex items-center justify-between rounded-xl border px-2.5 py-2 text-[12px] font-semibold transition-colors ${
                            ...
                            >
                                ...
                                {form.status === opt.value ? <Check size={13} /> : null}
                            </button>
                        ))}
                    </div>
                ...
                <div className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
                    ...
                    {categories.length === 0 ? (
                        <p className="mt-2 text-[11px] text-slate-400">
                            No categories yet — add them from the categories page.
                        </p>
                    ) : null}

                    <h3 className="mt-5 text-[13px] font-bold uppercase tracking-widest text-slate-400">
            ...
            {/* SEO card (full width, below editor) */}
            <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm">
