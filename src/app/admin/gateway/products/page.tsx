...
import { adminGatewayService, CatalogProduct, GATEWAY_CONFIG, ENV_CONFIG } from '@/services/admin/gatewayService';
...
export default function GatewayProductsPage() {
    ...
    const fetch = useCallback(async () => {
        try {
            setLoading(true);
            setData(await adminGatewayService.getProducts(gateway || undefined, env || undefined));
        } catch (e) {
        ...
        <AdminLayout>
            <div className="p-8 max-w-[1400px] mx-auto space-y-6 pb-20">
                <div>
                    ...
                    <p className="text-[14px] text-slate-500 mt-0.5">Products synced to the payment gateway per plan.</p>
                </div>
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-slate-50/60 border-b border-slate-100">
                                    ...
                                    <th className="px-6 py-3.5 text-[11px] font-bold text-slate-400 uppercase tracking-widest">External Product ID</th>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {loading ? (
                                    ...
                                    <tr><td colSpan={6} className="px-6 py-20 text-center text-[13px] text-slate-400">No products synced</td></tr>
                                ) : data.map(p => (
                                    <tr key={p.id} className={`hover:bg-slate-50/60 transition-colors ${!p.is_active ? 'opacity-50' : ''}`}>
                                        ...
                                        <td className="px-6 py-4">
                                            <span className="text-[13px] font-bold text-slate-900">
                                                {p.plan_name ?? <span className="text-slate-400 italic font-normal">Unknown plan</span>}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className="text-[11px] font-mono text-slate-600 select-all">{p.external_product_id}</span>
                                        </td>
                                        <td className="px-6 py-4">
                                            {p.is_active ? (
